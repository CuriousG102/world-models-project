...
def vae_loss_function(
    ...
    batch_size, sequence_length = reward.size()

    reward_loss = 0
    death_loss = 0
    for j in range(sequence_length):
        position_predicted_reward = predicted_reward[:, j:j+M]
        position_reward = reward[:, j:j+M]
        position_reward_loss = F.mse_loss(
            position_predicted_reward, position_reward, reduction='none')

        position_predicted_death = predicted_death[:, j:j+M]
        position_death = death[:, j:j+M]
        ...
        reward_loss += ((position_reward_loss.sum(axis=1) / gamma_sum).mean()) / sequence_length

        position_death_loss = position_death_loss * gamma_decay
        death_loss += ((position_death_loss.sum(axis=1) / gamma_sum).mean()) / sequence_length

    loss = reconstruction + kld + reward_loss + death_loss
